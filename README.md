# Luke's build of st - the simple (suckless) terminal with some minor changes from Kalelzar
...
+ **follow urls** by pressing `M-l`
+ **copy the output of commands** with `M-o`

## Bindings for


+ **scrollback** with `M-↑/↓` or `M-pageup/down` or `shift` while scrolling the mouse
+ **zoom/change font size**: same bindings as above, but holding down shift as well. `M-home` returns to default
+ **copy text** with `M-c`, **paste** is `M-v` or `shift-insert`
